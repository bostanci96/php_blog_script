...
if ($_GET["p"] == "haberkategori_edit")
{
 ...
 $kategori_urlxxx = p("kategori_url");
    ...
    else
    {
        ...
        {
            require_once ("app-upload/app.upload.php");
            require_once ("app-upload/haberkategori_resim_edit.php");
        }
         if (strlen($dosya2) > 3)
        {
            require_once ("app-upload/app.upload.php");
            require_once ("app-upload/haberkategori_sabitresim_edit.php");
        }
...
$katkonum_yeni = p("katkonum_yeni");
    ...
    else
    {
        ...
        if ($insert->rowCount())
        {
            ...
            {
                require_once ("app-upload/app.upload.php");
                require_once ("app-upload/haberkategori_resim_add.php");
                if ($dosya2) {
                    require_once ("app-upload/haberkategori_sabitresim_add.php");
                }
            }
            echo 'basarili';
        }
...
if ($_GET["p"] == "tek_habercat_sabit")
{
    $id = p("id");
      $resettt = $db->query("UPDATE haberkategori SET def_kat_ayar=1");
     if ($resettt->rowCount()){
    $kontrol = $db->query("SELECT * FROM haberkategori WHERE kategori_id='$id'");
    if ($kontrol->rowCount())
    {
            $update = $db->query("UPDATE haberkategori SET def_kat_ayar=2 WHERE kategori_id='$id'");
            if ($update->rowCount())
            {
                echo 'basarili';
            }else{
