...
for($i=0; $i<$toplam; $i++){
	...
	$handle->file_new_name_body = $newName;
				...
				/* Büyük resmi yükle */
				$handle->Process("../images/burclar/big/");
				if($handle->processed){
					$handle->file_overwrite = true;
					$handle->image_resize = true;
					$handle->image_x = 360;
					...
					$handle->file_new_name_body = $newName;
					$handle->Process("../images/burclar/thumb/");
	$handle->file_overwrite = true;
					$handle->image_resize = true;
					$handle->image_x = 640;
					$handle->image_y = 360;
					$handle->image_ratio_crop=true;
					$handle->file_new_name_body = $newName;
					$handle->Process("../images/burclar/640x360/");
					$handle->file_overwrite = true;
					$handle->image_resize = true;
					$handle->image_x = 220;
					$handle->image_y = 270;
					...
					$handle->image_ratio_crop=true;
					$handle->file_new_name_body = $newName;
					$handle->Process("../images/burclar/860x510/");
					$handle->Process("../images/burclar/220x270/");
