...
for($i=0; $i<$toplam; $i++){
	...
	$handle->file_new_name_body = $newName;
				...
				/* Büyük resmi yükle */
				$handle->Process("../images/duzsayfa/big/");
				if($handle->processed){
					$handle->image_resize = true;
					$handle->image_x = 270;
					$handle->image_y = 190;
					...
					$handle->file_new_name_body = $newName;
					$handle->Process("../images/duzsayfa/thumb/");
					...
					$handle->image_ratio_crop=true;
					$handle->file_new_name_body = $newName;
					$handle->Process("../images/duzsayfa/220x270/");
					$handle->image_resize = true;
					$handle->image_x = 370;
					$handle->image_y = 480;
					$handle->image_ratio_crop=true;
					$handle->file_new_name_body = $newName;
					$handle->Process("../images/duzsayfa/860x510/");
						$handle->image_resize = true;
					$handle->image_x = 640;
					$handle->image_y = 360;
					$handle->image_ratio_crop=true;
					$handle->file_new_name_body = $newName;
					$handle->Process("../images/duzsayfa/640x360/");

					$imgInsert = $db->query("UPDATE duzsayfa SET  
