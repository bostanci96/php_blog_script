...
function Kategori_Select($tree,$level=0){
	global $db;
	$sorgula = $db->query("SELECT * FROM haberkategori WHERE kategori_durum=1 ORDER BY kategori_adi ASC");
	if($sorgula->rowCount()){
		foreach ($sorgula as $item)
		{
			echo '<option value="'.URL.HBURL.'/'.$item["kategori_id"].'_'.$item["kategori_url"].'/">'.str_repeat('-', $level*3).$item['kategori_adi'].'</option>';
		}
	}
}
function Kategori_Selectxxyy($tree,$level=0){
	global $db;
	$sorgula = $db->query("SELECT * FROM galerikategori WHERE kategori_durum=1 ORDER BY kategori_adi ASC");
	if($sorgula->rowCount()){
		foreach ($sorgula as $item)
		{
			echo '<option value="'.URL.GBURL.'/'.$item["kategori_id"].'_'.$item["kategori_url"].'/">'.str_repeat('-', $level*3).$item['kategori_adi'].'</option>';
		}
	}
}
?>
	...
	$(document).ready(function(){
		...
		$("select[name=hrefTypes]").change(function(){
			...
			if(hrefType=='manuel'){
                ...
                $("#galKategoriler").hide(500);
            	$("#galKategoriler select[name=menu_href]").attr('disabled','disabled');
            ...
            else if(hrefType=="galkategoriler"){
            	$("#hrefSayfalar").hide(500);
            	$("#hrefSayfalar select[name=menu_href]").attr("disabled","disabled");
            	$("#hrefManuel").hide(500);
            	$("#hrefManuel input[name=menu_href]").attr('disabled','disabled');
            	$("#hrefKategoriler").hide(500);
            	$("#hrefKategoriler select[name=menu_href]").attr('disabled','disabled');
            	$("#galKategoriler").show(500);
            	$("#galKategoriler select[name=menu_href]").removeAttr('disabled');
              }
        });
								...
								<form role="form" class="form-horizontal" id="forms" method="POST" action="ajax.php?p=menu_add">
									<div class="form-body">
										<div class="row">
													...
													<div class="col-md-10">
														<fieldset class="form-group">
															<select class="form-control" id="basicSelect" name="menu_type">
																<option value="0">Header Ana Menü</option>
																<option value="5">Header Sub Menü</option>
																<option value="1">Header Kayar Menü</option>
																<option value="6">Mobil Kayar Menü</option>
																<option value="2">Footer Üst Menü</option>
																<option value="3">Footer Listeli Menü</option>
															<!--	<option value="4">Taban Menü</option>   -->                                                       
															</select>
														</fieldset>
													</div>
												</div>
												...
												<div class="form-group row">
													...
													<div class="col-sm-9">
														<select name="hrefTypes" class="form-control">
															...
															<option value="kategoriler" >Makale Kategorileri Göster</option>
															<option value="galkategoriler" >Galeri Kategorileri Göster</option>
														</select>
													</div>
												</div>
											</div>
													...
													<select name="menu_href" class="form-control"  disabled>
														...
														if($sayfaQuery->rowCount()){
															foreach($sayfaQuery as $sayfaRow){
																echo '<option value="'.URL.SAYFA.$sayfaRow["sayfa_url"].'/">'.$sayfaRow["sayfa_adi"].'</option>';
															}
														}
														?>
													</select>
											...
											<div class="col-12" id="galKategoriler" style="display:none"> 
												<div class="form-group row">
													<label for="input-text" class="col-sm-2 control-label">Menü Linki Belirleyin</label>
													<div class="col-sm-9">
														<select name="menu_href" class="form-control"  disabled>
															<option value="">Kategori Seçin !</option>
															<?php Kategori_Selectxxyy(0,0);?>
														</select>
													</div>
												</div>
											</div>
