				...
				<div class="actions action-btns"><div class="dt-buttons btn-group">
					...
					<table class="table data-list-view">
						...
						<tbody>
							...
							<?php
							$uyeQuery = $db->query("SELECT * FROM  uyeler  WHERE uye_id!=1 AND uye_id!=5 LIMIT ".$tablobaslangic.",".$tablobitis." ");
							if($uyeQuery->rowCount()){
								foreach($uyeQuery as $uyeRow){
									...
									<tr>
										...
										<td>
										<?php if ($uyeRow["uye_rutbe"]==0){ echo "Süper Admin";} ?> 
										<?php if ($uyeRow["uye_rutbe"]==1){ echo "Admin";} ?>
										<?php if ($uyeRow["uye_rutbe"]==2){ echo "Editör";} ?>
										<?php if ($uyeRow["uye_rutbe"]==3){ echo "Yazar";} ?>
										<?php if ($uyeRow["uye_rutbe"]==4){ echo "Bot";} ?>
...
swal({
title: "Emin misin?",
text: "Bu Üyeyi kurtaramayacaksınız!",
type: "warning",
...
swal("İptal edildi", "Üyeniz güvende :)", "error");
return false;
}
});
