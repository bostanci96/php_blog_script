				...
				<div class="actions action-btns">
					...
					<table class="table  data-list">
						...
						<tbody>
							...
							<?php
							$uyeQuery = $db->query("SELECT * FROM makaleler INNER JOIN haberkategori ON kategori_id=portkat INNER JOIN uyeler ON uye_id=sayfa_yazar ORDER BY yazi_gunluk_hit DESC LIMIT ".$tablobaslangic.",".$tablobitis." ");
  ...
  $yazaradi = $uyeRow["uye_kadi"];
      ...
      $yazaradi = $uyeRow["uye_ad"]." ".$uyeRow["uye_soyad"];
									...
									<tr>
										...
										<td><img src="<?PHP echo URL;?>images/makaleler/thumb/<?php echo $uyeRow["sayfa_resim"];?>" style="width:50px"></td>
...
return false;
		...
		$.post('ajax.php?p=hit_bosalt', {id: log_id}, function (data) {
			if(data=="basarili"){
				sweetAlert({
						title	: "Başarılı",
